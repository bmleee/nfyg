...
class Ranking extends Component {
  ...
  closeModal = () => {
	...
	_onClickAddPost = async () => {
		// await add new post
		try {
			let { abstract: { productName } } = this.props
			...
			let { response } = await createPost({productName, title, content, thresholdMoney, isDirectSupport})
	...
	render() {
		...
		const canEdit2 = appUtils.getUser().canEdit

		let condition1 = "전체공개";
		let condition2 = "후원자공개";
		
		// console.log('this.state.visible', this.state.visible)
		// console.log('postgetuser', postgetuser);
		...
		const item = posts.map(({
			...
			title,
			created_at,
			created_at_new = !created_at ? null : date2string(created_at),
			post,
			comments,
			content,
			thresholdMoney
