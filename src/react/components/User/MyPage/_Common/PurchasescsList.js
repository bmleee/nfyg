...
export default class PurchasescsList extends Component {
  render() {
    const {
    ...
    var success_purchase_num = new Array;
		for(var i in purchases) {
		  if(purchases[i].purchase_info.purchase_state == "scheduled") {
			  success_purchase_num.push(purchases[i].purchase_info.purchase_state)
		  }
		}

    return (
      <div className="purcahse-list-container">
        <h4 className="purcahse-list-title">후원 완료 내역</h4>
        {
          success_purchase_num.length > 0
            ? purchases.map(({
              _id,
              project,
              product,
