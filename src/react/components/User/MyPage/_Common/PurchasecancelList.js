...
export default class PurchasecancelList extends Component {
  render() {
    const {
    ...
    var cancel_purchase_num = new Array;
		for(var i in purchases) {
		  if(purchases[i].purchase_info.purchase_state == "cancel-by-user") {
			  cancel_purchase_num.push(purchases[i].purchase_info.purchase_state)
		  }
		}

    return (
      <div className="purcahse-list-container">
        <h4 className="purcahse-list-title">후원 취소 내역</h4>
        {
          cancel_purchase_num.length > 0
            ? purchases.map(({
              _id,
              project,
              product,
